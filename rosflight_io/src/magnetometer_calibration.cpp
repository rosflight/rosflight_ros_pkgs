...
MagnetometerCalibrator::MagnetometerCalibrator(float accel_orientation_threshold,
                                               int consecutive_orientation_threshold,
                                               float lpf_alpha)
{
...
Eigen::Matrix3d MagnetometerCalibrator::get_soft_iron_correction() { return soft_iron_correction_; }

Eigen::Vector3d MagnetometerCalibrator::get_hard_iron_offset() { return hard_iron_offset_; }
...
void MagnetometerCalibrator::calibrate()
{
  ...
  correction_matrix << coeffs(0), coeffs(1), coeffs(2), coeffs(1), coeffs(3), coeffs(4), coeffs(2),
    coeffs(4), coeffs(5);
  ...
  float third_root_determinant =
    std::cbrt(determinant); // Scaling term. Adjusts the result close to the unit sphere.
  ...
  soft_iron_correction_ =
    e_solver.eigenvectors() * sqrt_e_values.asDiagonal() * e_solver.eigenvectors().transpose();
}
...
void MagnetometerCalibrator::check_orientation_data()
{
  ...
  if (((mag_calibration_data_.rows() - start_of_current_orientation_data_)
       / (num_times_data_checked_ * rotation_complete_check_interval))
        >= 1
      && !all_orientation_data_gathered()) {

    num_times_data_checked_ += 1;

    Eigen::MatrixXd current_orientation_data = mag_calibration_data_.bottomRows(
      mag_calibration_data_.rows() - start_of_current_orientation_data_);
    ...
    for (int i = 0; i < num_bins; i++) {
      binned_values.push_back(
        centered_data.middleRows(bin_index_start, length_of_bin).colwise().mean());
...
void MagnetometerCalibrator::find_orientation()
{
  ...
  if (consecutive_orientation_ >= consecutive_orientation_threshold_
      && !orientation_covered(current_orientation_)) {
