...
namespace mavrosflight
{
  ...
  public:
    ...
    std::tuple<bool, uint8_t> get_device_from_str(const std::string &name);
    std::tuple<bool, uint8_t> get_config_from_str(uint8_t device, const std::string &name);
  ...
  private:
    ...
    std::string make_internal_name(const std::string &name);
    std::vector<std::string> get_words(const std::string &internal_name);
