...
class ParamTuning(Plugin):
    def __init__(self, context):
        ...
        if args.config_filepath is None:
            ...
            filename, _ = QFileDialog.getOpenFileName(
                None, 'Open Configuration File', '', 'YAML Files (*.yaml)', options=options
            )
            if not filename:
                self._node.get_logger().fatal(
                    'No configuration file selected, please provide a configuration file like'
    ...
    def add_arguments(parser):
        ...
        group.add_argument(
            '--config-filepath', type=str, help='Path to the .yaml GUI configuration file'
        )
        group.add_argument(
            '--param-filepath',
            type=str,
            help='Path to the ROS .yaml parameter file, to save the parameters to',
        )
