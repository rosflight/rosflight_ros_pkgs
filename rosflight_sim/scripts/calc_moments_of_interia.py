...
def calc_Jcg(
    positions: np.ndarray, masses: np.ndarray, inertias: list, cg: np.ndarray
) -> np.ndarray:
    Js = np.zeros((3, 3))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Script to compute the inertia tensor given point masses from yaml file'
    )
    parser.add_argument(
        '-f', '--param-file', type=str, default=None, help='Location of the config file. Required'
    )
    parser.add_argument(
        '-o',
        '--output-file',
        type=str,
        default=os.path.join(os.getcwd(), 'inertia_tensor.yaml'),
        help='Location of the output file. Default is current working directory',
    )
